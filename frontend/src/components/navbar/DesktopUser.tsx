...
function DesktopUser({ userMenuRef, userMenuOpen, setUserMenuOpen }: DesktopUserPropsTypes) {
    return (
        <div ref={userMenuRef} className="relative hidden lg:block">
            {1 ? (
                ...
                <>
                    <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-3 rounded-xl border border-transparent px-2 py-1.5 transition-all hover:border-slate-200 hover:bg-slate-50">
                    ...
                    {userMenuOpen && (
                        <div className="absolute left-0 top-[calc(100%+10px)] w-60 overflow-hidden rounded-2xl border border-slate-100 bg-white p-2 shadow-xl shadow-slate-900/10">
                            ...
                            <button className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-red-500 transition-colors hover:bg-red-50">
                                <FiLogOut size={18} />
                                خروج
                            </button>
                        </div>
